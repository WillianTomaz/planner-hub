...
export const EditModal: React.FC<EditModalProps> = ({
  ...
  // Update form data when initialData changes and focus first input
  useEffect(() => {
    if (isOpen) {
  ...
  const renderField = (field: EditField, isFirstField: boolean = false) => {
    const commonProps = {
      id: field.name,
      name: field.name,
      ...
      case 'checkbox':
        return (
          <input
            ...
            className={`modal-checkbox ${errors[field.name] ? 'error' : ''}`}
            ref={isFirstField ? firstInputRef : undefined}
          />
        );
